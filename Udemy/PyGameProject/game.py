...
from constanse import SCREEN_WIDTH, SCREEN_HEIGHT, SCREEN_FILL_COLOR, GAME_CAPTION
...
class Game:
        ...
        while self.game_is_running:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    sys.exit()
                self.handel_key_events(event)
            self.update_game_state()
            self.draw_screen()
        self.show_game_over()

    def handel_key_events(self, event):
    ...
    def update_game_state(self):
        ...
        if self.ball.is_collision(self.alien):
            ...
            self.ball.reset()
            self.game_score += 1
        if self.alien.has_reached_fighters(self.fighter):
